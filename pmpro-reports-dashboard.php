...
function init_pmpro_reports_dashboard()
{
	...
	if ( empty( $_REQUEST['pmpro_reports'] ) && strpos( $_SERVER['REQUEST_URI'], '/pmpro-reports-dashboard' ) !== 0 ) {
		return;
	}
...
function pmprordb_add_rewrite_rule() {
	add_rewrite_rule( 'pmpro-reports-dashboard', 'index.php?pmpro_reports=true', 'top' );
	flush_rewrite_rules();
}
